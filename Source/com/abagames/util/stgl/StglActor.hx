...
class StglActor
{
	...
	public static function add(name:String, args:Array<Float> = null, parent:StglActor = null):StglActor
	{
	...
	public function getReadOnlyVariableValue(name:String):Float
	{
		switch (name)
		{
			...
			case "$distanceToPlayer", "$dtp":
				...
				return Math.sqrt(ox * ox + oy * oy) / Math.min(stglManager.screenWidth, stglManager.screenHeight);
			case "$ticks", "$t":
				return ticks;
			case "$parentX", "$prx":
				if (parent == null)
