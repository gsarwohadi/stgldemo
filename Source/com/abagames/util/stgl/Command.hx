...
class Command
{
	public static var s:Map<String, Command>;
	
	static inline function COMMAND_EREG()
	{
		return ~/^([@\$:]?\w+)(?:\s*)(<?[\+\-]?=>?)?(?:\s+)?(.*)$/;
	}
	...
	public static function read(string:String) 
	{
		...
		for (line in lines) 
		{
			...
			try 
			{
				current = new Command(currentActorType, commandName, commandOperator, args, lineCount);
			}
			catch (msg:String)
			{
	...
	static function addRootCommand(name:String, command:Command):Void 
	{
		...
		{
			command.addChildToHead(new Command(command.actorType, "@y", "<=", ["$parentY"], -1));
			command.addChildToHead(new Command(command.actorType, "@x", "<=", ["$parentX"], -1));
		}
		if (command.actorType == ActorType.Stage || command.actorType == ActorType.Turret) 
		{
			command.addChildToTail(new Command(command.actorType, "vanish", null, null, -1));
		}
	}
	...
	public function new(actorType:ActorType, commandName:String, commandOperator:String, argStrs:Array<String>, lineCount:Int) 
	{
		...
		if (commandOperator != null && commandOperator.length > 0) 
		{
			...
			{
				case "=": 
					type = Var(commandName, Assignment);
				case "+=": 
					type = Var(commandName, Add);
				case "-=": 
					type = Var(commandName, Sub);
				case "<=": 
					type = Var(commandName, ConsistentlyAssignment);
				case "<+=": 
					type = Var(commandName, ConsistentlyAdd);
				case "<-=": 
					type = Var(commandName, ConsistentlySub);
				case "=>": 
					type = Var(commandName, Target);
				default: 
