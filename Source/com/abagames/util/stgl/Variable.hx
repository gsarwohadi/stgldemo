...
class Variable
{
	...
	public function update(actor:StglActor):Void
	{
		...
		if (consistentlyValue != null)
			value = consistentlyValue.calc(actor);
			
		if (consistentlyAdd != null)
			value += consistentlyAdd.calc(actor);
			
		if (consistentlySub != null)
			value -= consistentlySub.calc(actor);
	}
}