...
class Expression
{
	...
	static inline function PARSED_PATTERN()
	{
		return new EReg("^" + PARSED_PREFIX_STRING + "(\\d+)" + PARSED_POSTFIX_STRING + "$", "");
	}
	static inline function PARSED_PATTERN_MATCH()
	{
		return new EReg(PARSED_PREFIX_STRING + "(\\d+)" + PARSED_POSTFIX_STRING, "");
	}
	static inline function IN_BRACKETS_PATTERN()
	{
		return "\\(([^()]+)\\)";
	}
	static inline function FUNCTION_PATTERN()
	{
		return new EReg("(sin|cos)" + IN_BRACKETS_PATTERN(), "");
	}
	static inline function BRACKETS_PATTERN()
	{
		return new EReg(IN_BRACKETS_PATTERN(), "");
	}
	static inline function VALUE_PATTERN()
	{
		return "(\\d+(?:\\.\\d+)?|[@\\$:]?\\w+)";
	}
	static inline function UNARY_PATTERN()
	{
		return new EReg("([!\\._\\|*\\/%\\+\\-<>=%&\\?:]|^)([\\-!])" + VALUE_PATTERN(), "");
	}
	static inline function RANDOM_PATTERN()
	{
		return new EReg(VALUE_PATTERN() + "(\\.\\.|__|\\|)" + VALUE_PATTERN(), "");
	}
	static inline function MULDIV_PATTERN()
	{
		return new EReg(VALUE_PATTERN() + "([*\\/%])" + VALUE_PATTERN(), "");
	}
	static inline function ADDSUB_PATTERN()
	{
		return new EReg(VALUE_PATTERN() + "([\\+\\-])" + VALUE_PATTERN(), "");
	}
	static inline function COMPARE_PATTERN()
	{
		return new EReg(VALUE_PATTERN() + "([<>]=?)" + VALUE_PATTERN(), "");
	}
	static inline function EQUAL_PATTERN()
	{
		return new EReg(VALUE_PATTERN() + "([!=]=)" + VALUE_PATTERN(), "");
	}
	static inline function LOGIC_PATTERN()
	{
		return new EReg(VALUE_PATTERN() + "(&&|\\|\\|)" + VALUE_PATTERN(), "");
	}
	static inline function TERNARY_PATTERN()
	{
	...
	function searchFunction(string:String, ereg:EReg, pushOperatorFunc:String -> Array<Operator> -> Void):String
	{
	...
	function searchUnaryOpeator(string:String, ereg:EReg, pushOperatorFunc:String -> Array<Operator> -> Void):String
	{
		while (ereg.match(string))
		{
	...
	function searchOpeator(string:String, ereg:EReg, n:Int, pushOperatorFunc:String -> Array<Operator> -> Void):String
	{
		while (ereg.match(string))
		{
